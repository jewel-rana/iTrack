            ...
            <div class="tab-content" id="myTabContent">
              <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                    <table class="table table-striped table-bordered table-sm">
                      ...
                      <tbody>
                        <tr v-for="packege in packeges.data" :key="packege.id">
                          ...
                          <td>
                            <router-link to="/dashboard/packege/view/:key" class="btn btn-primary btn-sm"><span class="fa fa-eye"></span></router-link>
                            <button @click="editModal( packege )" class="btn btn-success btn-sm"><span class="fa fa-edit"></span></button>
                            <button @click="deletePackege( packege.id )" class="btn btn-danger btn-sm"><span class="fa fa-times"></span></button>
                          </td>
                        </tr>
                      </tbody>
                    </table>
              ...
              <div class="modal-dialog" role="document">  
                    <div class="modal-content">
                      ...
                      <div class="modal-header modal-header-colored">
                        <h3 v-show="!editmode" class="modal-title">Create new packege</h3>
                        <h3 v-show="editmode" class="modal-title">Update packege</h3>
                      ...
                      <div class="modal-body form">
                        <div class="form-group">
                          <input v-model="form.name" type="text" name="name" placeholder="Packege Name" 
                      ...
                      <div class="modal-footer">
                        ...
                        <button v-show="!editmode" class="btn btn-success" type="submit">Save packege</button>
                        <button v-show="editmode" class="btn btn-success" type="submit">Update packege</button>
                      </div>
                      </form>
                    </div>
                  </div>
...
<script>
    export default {
        ...
        methods: {
          load(){
            axios.get('/api/packege').then(( data ) => { this.packeges = data.data } );
          },
          createModal(){
            this.editmode = false;
            this.form.reset();
          ...
          create(){
            this.$Progress.start();
            this.form.post('/api/packege').then((response) => {
              if( response.data.success == true ) {
                ...
                toast({
                  type: 'success',
                  title: 'Packege has been created.'
                });
          ...
          update(){
            this.$Progress.start();
            this.form.put('/api/packege/' + this.form.id).then(() => {
              //form is success
              $('#appModal').modal('hide');
              //show notification
              toast({
                type: 'success',
                title: 'Packege information updated.'
              });
